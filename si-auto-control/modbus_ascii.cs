...
namespace si_auto_control
{
    class modbus_ascii
    {
        ...
        public bool[] modbusASCII_readCoils(Byte slaveAddr, UInt16 regAddr, UInt16 numCoils)
        {
            if (numCoils > 1000)
                throw new ArgumentOutOfRangeException(nameof(numCoils), "Too many coils requested");

            byte[] modbus_buf = new byte[7];

            modbus_buf[0] = slaveAddr;  // slave address
            modbus_buf[1] = 1;          // function code (1)
            modbus_buf[2] = (byte)(regAddr >> 8);   // regAddr MSB
            modbus_buf[3] = (byte)(regAddr & 0xff); // regAddr LSB
            modbus_buf[4] = (byte)(numCoils >> 8);   // regCnt MSB
            modbus_buf[5] = (byte)(numCoils & 0xff); // regCnt LSB
            ...
            byte[] readBytes = StringToByteArray(modbusStr.Substring(modbusStr.IndexOf(':') + 1));


            int bytes = (numCoils + 7) / 8;	// Round upwards

            bool[] retval = new bool[numCoils];
            ...
            for (int i=0; i<bytes; i++)
            {
                for (int bitpos = 0; bitpos < 8; bitpos++)
                {
                    ...
                    retval[coilIdx++] = ((readBytes[3 + i] >> bitpos) & 0x01) == 0x01;
        ...
        public bool modbusASCII_writeCoil(Byte slaveAddr, UInt16 regAddr, bool state)
        {
            ...
            modbus_buf[0] = slaveAddr;  // slave address
            modbus_buf[1] = 5;          // function code (5)
            modbus_buf[2] = (byte)(regAddr >> 8);   // regAddr MSB
            modbus_buf[3] = (byte)(regAddr & 0xff); // regAddr LSB

            if (state)
            {
                modbus_buf[4] = 0xFF;   // state MSB
            }
            else
            {
                modbus_buf[4] = 0x00;   // state MSB
            }

            modbus_buf[5] = 0; // state LSB
            modbus_buf[6] = modbus_LRC(modbus_buf, 1, 5);   // LRC
        ...
        public byte[] modbusASCII_readReg(Byte slaveAddr, UInt16 regAddr, UInt16 regCnt)
        {
            if (regCnt > 125)
            ...
            if (modbus_resp.Length != (regCnt*2) + 4)
            {
                // Length wrong
                return null;
            }

            if (modbus_resp[2] != (regCnt * 2))
            {
                // Length wrong
                return null;
            }

            byte LRC = modbus_LRC(modbus_resp, 0, modbus_resp.Length - 1);


            if (LRC != modbus_resp[modbus_resp.Length - 1])
            {
                // LRC wrong
                return null;
            }
            ...
            byte[] retBytes = new byte[modbus_resp.Length - 4];

            Array.Copy(modbus_resp, 3, retBytes, 0, modbus_resp.Length - 4);

            return retBytes;

        }
        public byte[] modbusASCII_readHoldReg(Byte slaveAddr, UInt16 regAddr, UInt16 regCnt)
        {
            if (regCnt > 125)
                throw new ArgumentOutOfRangeException(nameof(regCnt), "Too many registers requested");
        ...
        public bool modbusASCII_writeHoldRegs(Byte slaveAddr, UInt16 regAddr, UInt16 regCnt, byte[] regData)
        {
            if (regCnt > 125)
                throw new ArgumentOutOfRangeException(nameof(regCnt), "Too many registers requested");
